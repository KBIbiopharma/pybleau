...
class TestFilterDataFrameAnalyzer(TestCase, UnittestTools):
    ...
    def test_filter_no_auto_apply(self):
        df = self.df2
        analyzer = DataFrameAnalyzer(source_df=df, filter_auto_apply=False)
        self.assertEqual(analyzer.filter_exp, "")
        assert_frame_equal(analyzer.filtered_df, analyzer.source_df)
    ...
    def test_subclass_filter_applied(self):
        class DFAnalyzer(DataFrameAnalyzer):
...
@skipIf(not BACKEND_AVAILABLE or not KIWI_AVAILABLE, msg)
class TestSummaryDataFrameAnalyzer(TestCase, UnittestTools):
    ...
    def test_compute_summary(self):
        analyzer = DataFrameAnalyzer(source_df=self.df)
    ...
    def test_special_summary_list(self):
        ...
        df = self.df2
        analyzer = DataFrameAnalyzer(source_df=df, summary_index=summary_index)
        summary = analyzer.compute_summary()
        self.assertEqual(summary.index.tolist(), summary_index)
        self.assertFalse(np.any(np.isnan(summary)))

        # Make a default one and make sure they match
        orig_summarizer = DataFrameAnalyzer(source_df=df)
...
@skipIf(not BACKEND_AVAILABLE or not KIWI_AVAILABLE, msg)
class TestDataFrameAnalyzer(TestCase, UnittestTools):
...
@skipIf(not BACKEND_AVAILABLE or not KIWI_AVAILABLE, msg)
class TestSortingDataFrameAnalyzer(TestCase, UnittestTools):
    ...
    def test_initial_index_sort(self):
        ...
        analyzer = DataFrameAnalyzer(source_df=unsorted_df)
    ...
    def test_turn_off_initial_index_sort(self):
        ...
        analyzer = DataFrameAnalyzer(source_df=unsorted_df, data_sorted=False)
    ...
    def test_filter_transformation(self):
        ...
        with self.assertRaises(UndefinedVariableError):
            with reraise_traits_notification_exceptions():
                DataFrameAnalyzer(source_df=df, filter_exp="A > 2")

        analysis = DataFrameAnalyzer(
    ...
    def test_plotter_selection_connected(self):
        df = self.df
        model = DataFrameAnalyzer(source_df=df)
...
@skipIf(not BACKEND_AVAILABLE or not KIWI_AVAILABLE, msg)
class TestDisplayingDataFrameAnalyzer(TestCase, UnittestTools):
    ...
    def test_truncating_data(self):
        df = self.df
        analyzer = DataFrameAnalyzer(source_df=df, num_displayed_rows=100)
        self.assertIs(analyzer.displayed_df, analyzer.filtered_df)
